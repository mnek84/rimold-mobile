...
export function InternalRouteScreen({ route }: Props) {
  ...
  useEffect(() => {
    const coords = [...stopsWithCoords.map((s) => ({ latitude: s.latitude!, longitude: s.longitude! })), ...polylineCoords];
  ...
  const openGoogleMapsNext = useCallback(() => {
    if (nextStop === null || nextStop.latitude === null || nextStop.longitude === null) {
      return;
    }
